...
\begin{latin}
	\begin{algorithm}
		...
		\begin{algorithmic}[1]
			...
			\Repeat
				\For{$i = 1$ to $l$}
					\State $x_i(k+1), \beta_i(k+1), \psi_i(k+1) \gets \arg \min_{x_i,\beta_i,\psi_i \in vert(X_i)} \sum_{t \in T}(x_i(K_2\pi_i-\eta_{1,t}+\eta_{2,t}) + K_1\pi_i\psi_{t,i} + \sum_{s \in S}\beta_{t,s,i}(\nu_{2,t,s} - \nu_{1,t,s}))
					s. to: 
					$
