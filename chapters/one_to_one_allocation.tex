\chapter{اختصاص منابع پردازشی در شبکه اینترنت اشیاء}\label{chap:system_model}
  ...
  \section{مقدمه}
    ...
    در این نوع تخصیص منابع، تعدادی وظیفه و منبع پردازشی داریم که وظیفه‌ها باید برای پردازش داده‌های جمع‌آوری شده توسط حسگر‌های خود از منابع پردازشی موجود در شبکه استفاده کنند.
  ...
  \section{مدل سیستم}
    ...
    \cref{fig:system_model} مثالی از مدل سیستم این فصل را برای یک سرویس که دارای دو حسگر می‌باشد نشان می‌دهد.
    مجموعه‌ی گره‌های لایه‌ی ابری، مه، لبه و حسگری را به ترتیب با  $E$، $F$، $C$ و $S$ نمایش می‌دهیم. همچنین مجموعه منابع موجود در هر گره پردازشی را با $R$ نمایش می‌دهیم. 
    \begin{subequations}
    	\begin{align}
    	...
    	\end{align}
    \end{subequations}
    در مورد منابع پردازشی موجود در هر گره به عنوان مثال می‌توان گفت که $\sigma_c^r$ برابر است با مجموع ظرفیت پردازشی از منبع $r \in R$ در گره $c \in C$. مجموعه‌ی وظیفه‌های موجود در شبکه را با $T$ نشان می‌دهیم. 
    \begin{equation}
    	T = \{t_1, t_2, ..., t_{|T|}\}
    \end{equation}
	که در آن هر وظیفه به صورت زیر بیان می‌شود.
	\begin{equation}
	t \in T => t = (w_t, \delta_t, N_t, f_t^r(\lambda_t))
	\end{equation}
	در عبارت فوق $w_t$، $N_t$ و $\delta_t$ به ترتیب میزان پردازش مورد نیاز، بیشترین تعداد از وظیفه‌های مشابه و همچنین حداقل زمان مورد نیاز پردازشی برای وظیفه $t \in T$ را نشان می‌دهد.
	
	در مورد هزینه‌های پردازشی نیز به صورت زیر عمل می‌شود که $\pi_c$ میزان واحد هزینه پردازشی در گره $c \in C$ نشان می‌دهد. 
...
درواقع متغیر \cref{eqn:input_poission_rate} به عنوان ورودی مسئله تعریف می‌شود و در جریان حل مسئله مقدارش تعیین نمی‌شود. 
	...
	\begin{subequations}\label{eqn:psi_constraint}
		...
		\end{align}
	\end{subequations}
	حال با تعریف متغیر \cref{eqn:psi_def} می‌توان قید موجود در \cref{eqn:load_managing} را به صورت \cref{eqn:load_managing_new} دوباره نویسی کرد. 
...
همانطور که در قسمت مقدمه گفته شد برای محاسبه‌ی تاخیر صف در یک سیستم نوع اول دو متغیر نرخ ورود بسته‌ها $\lambda$ و نرخ پردازش آن‌ها $\mu$ لازم است. در مدل ما نرخ ورود وظیفه‌ها در هر گره به صورت یک متغیر میانی تغریف شده است اما در مورد نرخ پردازش بسته‌ها به توجه به نوع بسته و همچنین میزان قدرت پردازشی گره رابطه‌ی \cref{eqn:mu_def} در نظر گرفته شده‌است. 
	...
	\begin{subequations}\label{eqn:delay_def}
		...
		\end{align}
	\end{subequations}
	در صورت استفاده از متغیر میانی \cref{eqn:psi_def} در قید \cref{eqn:delay_constraint_modified} می‌توان این قید را به صورت \cref{eqn:delay_constraint_last} به صورت نهایی نمایش داد. 
	...
	\begin{subequations}\label{eqn:scalable_constraint}
		\begin{align}
	...
	\begin{subequations}\label{eqn:queue_stability}
		\begin{align}
	...
	\begin{equation}\label{eqn:objective_func_final}
		\begin{align*}
